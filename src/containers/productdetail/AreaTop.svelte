    ...
    <div class="area_top flex items-start justify-between max-w-screen-lg mt-24 mr-auto ml-auto px-4 flex-wrap">
        ...
        <div class="area_item info">
            ...
            <div class="area_btn_box mt-4 flex items-center justify-between flex-wrap">
                ...
                <button type="button"
                        class="box-border text-white bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-3 rounded"
                        on:click={onClickAddCart}>
                    장바구니
                </button>
                {#if $userState && $userState.level === 0}
                    <a href="/upload-product?id={$productDetailItem.id}" use:link  class="bg-purple-500 hover:bg-purple-400 rounded px-4 py-2 mt-4  text-white">
                        수정하기
                    </a>
                {/if}
            </div>
